...
def fetch_data(url, max_depth, visited_urls=set(), depth=0):
    ...
    try:
        ...
        # Download images if the checkbox is checked
        if download_images:
...
def download_images_from_url(url):
    try:
        ...
        logging.error(f"An error occurred while downloading images: {str(e)}")
...
def save_image(img_url):
    try:
        ...
        logging.error(f"An error occurred while saving image: {str(e)}")
...
def toggle_image_download():
    global download_images
    download_images = image_check_var.get()
...
image_check = tk.Checkbutton(choice_frame, text="Download Images", variable=image_check_var, bg="#ADD8E6", command=toggle_image_download)  
image_check.grid(row=3, column=0, columnspan=3)

browse_button = tk.Button(choice_frame, text="Browse Output Folder", command=browse_folder, bg="#ADD8E6")  
browse_button.grid(row=4, column=0, columnspan=3)

fetch_button = tk.Button(choice_frame, text="Fetch Data Press Twice", command=fetch_and_display_data, bg="#ADD8E6")
fetch_button.grid(row=5, column=0, columnspan=3)

stop_display_button = tk.Button(choice_frame, text="Stop and Display Data", command=stop_and_display_data, bg="red")
stop_display_button.grid(row=5, column=4, columnspan=3, pady=(10, 0))
